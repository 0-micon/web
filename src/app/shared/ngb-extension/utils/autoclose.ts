...
function closest(element: HTMLElement, selector): HTMLElement {
  if (!selector) {
    return null;
  }
  ...
  if (type) {
    zone.runOutsideAngular(() => {
      const shouldCloseOnClick = (event: MouseEvent | TouchEvent) => {
        ...
        if (type === 'inside') {
          return (
            isContainedIn(element, insideElements) && matchesSelectorIfAny(element, insideSelector)
          );
        } else if (type === 'outside') {
          return !isContainedIn(element, insideElements);
        } /* if (type === true) */ else {
          return (
            matchesSelectorIfAny(element, insideSelector) || !isContainedIn(element, insideElements)
          );
        }
      };
      ...
      const mouseDowns$ = fromEvent<MouseEvent>(document, iOS ? 'touchstart' : 'mousedown').pipe(
        map(shouldCloseOnClick),
        takeUntil(closed$)
      );

      const closeableClicks$ = fromEvent<MouseEvent>(document, iOS ? 'touchend' : 'mouseup').pipe(
